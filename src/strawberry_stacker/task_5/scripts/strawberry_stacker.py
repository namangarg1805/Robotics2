...
class stateMoniter:
    ...
    def spawn_callback(self,data):
        ...
        global setpointsD2
        global alter
        #Avoiding Same row to both drones consecutively thus avoiding collision
        global d2prev
        global d1prev
...
def main():
    ...
    def publish_setpointD1(setD1,fast=-1):
        ...
        loop=abs((((setD1.pose.position.x - previous_xD1)**2) + ((setD1.pose.position.y - previous_yD1)**2) )**0.5)*2.5
    ...
    def publish_setpointD2(setD2,fast=-1):
        ...
        #Distance formulae for calculating loop value so that according to distance it publishes for certain period of time
        #Basically Done for improving stability after reaching the position
        loop=abs((((abs(setD2.pose.position.x) - previous_xD2)**2) + ((abs(setD2.pose.position.y) - previous_yD2)**2) )**0.5)*2.5
        ...
        while waycheck!=True:
            ...
            if b:
                ...
                if b:
                    ...
                    if b[idbox][0][0]<210 and b[idbox][0][0]>200 and b[idbox][0][1]<220 and b[idbox][0][1]>205:
                        print("Drone1 Approaching Box")
                        setD1.pose.position.z=-0.4
    ...
    def calculate_blue_blockD1(cellx,celly):
        ...
        if cellx==0:
            boxdrop_loc  = (15.2, -8.8+celly*1.23, 8)
        else:
            boxdrop_loc  = (14.85+cellx*0.9, -8.8+celly*1.23, 8)
        return boxdrop_loc
    
    def calculate_red_blockD1(cellx,celly):
        #Calculating location of Red cells according to Drone1
        if cellx==0:
            boxdrop_loc  = (58, 63.8+celly*1.23, 8)
        else:
            boxdrop_loc  = (57.5+cellx*1, 63.8+celly*1.23, 8)
        return boxdrop_loc   
    ...
    velD1 = TwistStamped()
    velD1.twist.linear.x = 1.8
    velD1.twist.linear.y = 0
    velD1.twist.linear.z = 0
    
    velD2 = TwistStamped()
    velD2.twist.linear.x = 1.8
...
#AS MORE SETPOINT ARE ADDED AT THE END OF LIST WHILE RUNNING
    ...
    def drone2():
        ...
        while setpointsD2:
            ...
            ourD2.pose.orientation.x=10
            if i==0:
                publish_setpointD2(ourD2,170)
            else:
                publish_setpointD2(ourD2)                
            print("Drone2 Row Approached : ",rowsD2[i])
            ourD2.pose.position.z=3
            ...
            if box_id==1:
                ...
                ourD2.pose.position.y=-2
                ourD2.pose.position.z=6
                publish_setpointD2(ourD2,30)
                ...
                publish_setpointD2(ourD2)
                ourD2.pose.position.z=1.5
                print("D2 Going Down")
                publish_setpointD2(ourD2,10)
                for _ in range(0,2):
                    ofb_ctl.setGripperD2(False)
                print("Drone2 Gripper Deactivated")
                if spacecheck[1]==2:
                    ourD2.pose.position.z=2.5
